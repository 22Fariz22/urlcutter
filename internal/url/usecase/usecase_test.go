...
func Test_useCase_Get(t *testing.T) {
	...
	repoPG.EXPECT().Get(ctx, nil, "my_short_url").Return("", sql.ErrNoRows)

	getURL, _ := UC.Get(ctx, nil, "my_short_url")
...
func Test_useCase_Save(t *testing.T) {
	...
	repoPG.EXPECT().Save(ctx, nil, "my_long_url", "my_short_url").Return("", nil)

	saveURL, err := UC.Save(ctx, nil, "my_long_url", "my_short_url")
	require.NoError(t, err)
